...
const searchPlaylists = async (spotifyApi: SpotifyWebApi, query: string, limit: number) => {
...
const useSearchPlaylists = (query: string, limit: number) => {
  ...
  return useQuery(['SEARCH_PLAYLISTS', query, limit], () => searchPlaylists(spotifyApi, query, limit));
};
