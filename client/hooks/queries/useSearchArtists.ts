...
const searchArtists = async (spotifyApi: SpotifyWebApi, query: string, limit: number) => {
...
const useSearchArtists = (query: string, limit: number) => {
  ...
  return useQuery(['SEARCH_ARTISTS', query, limit], () => searchArtists(spotifyApi, query, limit));
};
